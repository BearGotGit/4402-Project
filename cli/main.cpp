...
Or press the subscription number you would like to change to.
        ...
        case 5:
        { // case 5 //
            std::string e_first, e_last, e_department;
            double e_salary;

            std::cout << "Enter the first name of the employee: ";
            std::cin >> e_first;
            std::cin.ignore();

            std::cout << "Enter the last name of the employee: ";
            std::cin >> e_last;
            std::cin.ignore();

            std::cout << "Enter the department of the employee: ";
            std::cin >> e_department;
            std::cin.ignore();
            ...
            // Escape single quotes in user inputs
            e_first = EscapeSingleQuotes(e_first);
            e_last = EscapeSingleQuotes(e_last);
            e_department = EscapeSingleQuotes(e_department);

            // Build the SQL INSERT statement using sqlite3_mprintf to prevent SQL injection
            char *sql_insert_employee = sqlite3_mprintf(
                "INSERT INTO EMPLOYEE (EmployeeID, FirstName, LastName, Department, Salary) "
                "VALUES (NULL, '%q', '%q', '%q', %.2f);",
                e_first.c_str(), e_last.c_str(), e_department.c_str(), e_salary);

            // Execute the SQL statement
            rc = sqlite3_exec(db, sql_insert_employee, nullptr, 0, &zErrMsg);

            // Get the autogenerated EmployeeID
            int employee_id = sqlite3_last_insert_rowid(db);
            std::cout << "Assigned Employee ID: " << employee_id << std::endl;
            ...
            // Free the memory allocated by sqlite3_mprintf
            sqlite3_free(sql_insert_employee);
            break;
        }
        case 6:
        { // case 6 //
